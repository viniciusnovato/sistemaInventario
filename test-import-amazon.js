...
async function processItem(item, index) {
    try {
        ...
        console.log(`Número de Série: ${item['Número de Série']}`);
        ...
        console.log('📋 Dados preparados:', JSON.stringify(moduleData, null, 2));
        ...
        const { data, error } = await supabase
            ...
            .insert({
                ...
                metadata: { source: 'csv_import_test', created_by: 'system' },
                tenant_id: null,
                company_id: null,
                created_by: null,
...
async function testImport() {
    try {
        console.log('🧪 TESTE DE IMPORTAÇÃO - Primeiros 3 itens\n');
        ...
        // Processar apenas os primeiros 3 itens
        const items = [];
        const maxItems = Math.min(4, lines.length); // 3 itens + cabeçalho
        ...
        console.log(`🧪 Testando com ${items.length} itens:\n`);
        
        // Mostrar preview dos itens
        items.forEach((item, index) => {
            console.log(`${index + 1}. ${item.Nome} (${item['Número de Série']})`);
        });
        
        console.log('\n🚀 Iniciando teste de importação...\n');
        ...
        console.log(`📊 Total testado: ${items.length}`);
        ...
        if (results.success > 0) {
            ...
            console.log('✅ Você pode agora executar a importação completa com: node import-amazon-csv.js');
        } else {
            console.log('\n⚠️  Nenhum item foi importado no teste. Verifique os erros acima.');
        }

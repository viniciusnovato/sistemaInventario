    ...
    try {
        ...
        if (quantidade_inicial !== undefined && quantidade_inicial !== null) {
            ...
            if (parseFloat(quantidade_inicial) > 0) {
                ...
                console.log('[DEBUG QUANTIDADE ESTOQUE: ', quantidade_inicial, ']');
...
app.put('/api/laboratorio/produtos/:id', authenticateToken, async (req, res) => {
    try {
        ...
        if (quantidadeMin !== undefined || quantidadeMax !== undefined || quantidade_inicial !== undefined) {
            // Primeiro, verificar se já existe registro de estoque para este produto
            const { data: estoqueExistente, error: selectError } = await supabaseAdmin
            ...
            if (selectError && selectError.code !== 'PGRST116') { // PGRST116 = no rows returned
                console.error('❌ Erro ao buscar estoque existente:', selectError);
                throw selectError;
            }

            const quantidadeAtualAntes = estoqueExistente?.quantidade_atual || 0;
            
            // Debug: Log dos valores para verificar cálculo
            console.log('🔍 Debug estoque - Edição de produto:', {
                produto_id: req.params.id,
                quantidade_inicial_recebida: quantidade_inicial,
                quantidade_atual_antes: quantidadeAtualAntes,
                estoque_existente: !!estoqueExistente
            });

            // Construir objeto de dados do estoque
            const estoqueData = {
                produto_id: req.params.id,
                quantidade_minima: quantidadeMin !== undefined ? quantidadeMin : (estoqueExistente?.quantidade_minima || 0),
                quantidade_maxima: quantidadeMax !== undefined ? quantidadeMax : (estoqueExistente?.quantidade_maxima || null),
                atualizado_por: req.user.id
            };

            // Processar quantidade_inicial se fornecida
            let novaQuantidadeAtual = quantidadeAtualAntes;
            if (quantidade_inicial !== undefined && quantidade_inicial !== null && quantidade_inicial !== '') {
                const quantidadeNumerica = parseFloat(quantidade_inicial);
                if (!isNaN(quantidadeNumerica) && quantidadeNumerica >= 0) {
                    novaQuantidadeAtual = quantidadeNumerica;
                    estoqueData.quantidade_atual = novaQuantidadeAtual;
                    console.log('🔍 Nova quantidade_atual definida:', novaQuantidadeAtual);
                } else {
                    console.log('⚠️ Quantidade inicial inválida, mantendo estoque atual:', quantidade_inicial);
                }
            }
            ...
            if (estoqueError) {
                console.error('❌ Erro ao atualizar/inserir estoque:', estoqueError);
                throw estoqueError;
            }

            // Registrar movimentação apenas se houve mudança na quantidade atual
            if (quantidade_inicial !== undefined && novaQuantidadeAtual !== quantidadeAtualAntes) {
                const diferenca = novaQuantidadeAtual - quantidadeAtualAntes;
                ...
                const { error: movimentacaoError } = await supabaseAdmin
                    ...
                    .insert([{
                        ...
                        tipo: tipoMovimento,
                        quantidade: quantidadeMovimento,
                        motivo: 'Ajuste de estoque inicial via edição',
                ...
                if (movimentacaoError) {
                    ...
                    // Não falhar a operação por erro na movimentação, apenas logar
                }
            } else {
                console.log('ℹ️ Sem alteração na quantidade atual - não registrando movimentação');
            }
        }
